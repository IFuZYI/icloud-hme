...
describe('AliasesPage', () => {
  ...
  it('按 email/label 大小写不敏感搜索与 active 状态筛选', async () => {
    ...
    await user.click(screen.getByRole('option', { name: /已启用/ }))
    expect(screen.getByText('alpha@icloud.com')).toBeInTheDocument()
    expect(screen.queryByText('beta@icloud.com')).toBeNull()
  })
  ...
  it('创建时间默认倒序且可切换正序,兼容时间戳并按收件箱格式显示', async () => {
    ...
    const emailButtons = () => Array.from(table.querySelectorAll<HTMLButtonElement>('button.aliases-email'))
  ...
  it('停用别名:显示目标邮箱并二次确认', async () => {
    ...
    await user.click(within(alphaRow).getByRole('button', { name: /更多操作/ }))
  ...
  it('激活别名:显示目标邮箱并二次确认', async () => {
    ...
    const user = userEvent.setup()
    const betaRow = within(screen.getByRole('table')).getByRole('row', { name: /beta@icloud\.com/ })
    await user.click(within(betaRow).getByRole('button', { name: /更多操作/ }))
    await user.click(screen.getByRole('menuitem', { name: /激活/ }))
