...
describe('AccountsPage', () => {
  ...
  it('添加账号:host 只能选全球区或中国区', async () => {
    ...
    const user = userEvent.setup()
    await user.click(screen.getByRole('button', { name: /添加账号/ }))
    // 区域下拉: 打开后可见两个区域选项(SelectMenu 弹层)
    await user.click(screen.getByRole('button', { name: /选择区域/ }))
    expect(await screen.findByRole('option', { name: /全球区/ })).toBeInTheDocument()
    expect(screen.getByRole('option', { name: /中国区/ })).toBeInTheDocument()
    await user.click(screen.getByRole('option', { name: /中国区/ }))
